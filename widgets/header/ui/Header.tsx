    ...
    <header className={styles.header}>
      <div className={`container ${styles.container}`}>
        <Link href="/" className={styles.logo}>
          <Image src="/mebelnyy-magazin.png" alt="Мебельный магазин" width={120} height={40} />
        </Link>
            ...
            <div className={styles.mobileActions}>
              <Link href="/favorites" className={styles.mobileActionButton} onClick={() => setIsMenuOpen(false)}>
                ...
                {favoritesState.totalItems > 0 && (
                  <span className={styles.mobileBadge}>{favoritesState.totalItems}</span>
                )}
              </Link>
              ...
              <Link href="/cart" className={styles.mobileActionButton} onClick={() => setIsMenuOpen(false)}>
                ...
                <span>Корзина</span>
                {cartState.totalItems > 0 && <span className={styles.mobileBadge}>{cartState.totalItems}</span>}
              </Link>
            </div>
        ...
        <div className={styles.actions}>
          <Link href="/favorites" className={styles.actionButton} aria-label="Избранное">
            <div className={styles.iconWrapper}>
              ...
              {favoritesState.totalItems > 0 && <span className={styles.badge}>{favoritesState.totalItems}</span>}
            </div>
          </Link>
          <Link href="/cart" className={styles.actionButton} aria-label="Корзина">
            <div className={styles.iconWrapper}>
              ...
              {cartState.totalItems > 0 && <span className={styles.badge}>{cartState.totalItems}</span>}
            </div>
          </Link>
        </div>
      </div>
